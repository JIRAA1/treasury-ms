...
interface DialogState {
  ...
  onCancel?: () => void
  loading?: boolean
  show: (options: { type: DialogType; title: string; message: string; onConfirm?: () => void; onCancel?: () => void }) => void
  hide: () => void
...
export function GlobalDialog() {
  const { isOpen, type, title, message, onConfirm, onCancel, hide, loading } = useDialog()
      ...
      <div className="bg-background-secondary w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white/10 p-8 text-center">
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            disabled={loading}
            className={cn(
              "w-full py-3.5 rounded-2xl text-[14px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50",
              type === 'error' ? "bg-red-600 text-white" : "bg-brand text-white shadow-lg shadow-brand/20"
            )}
          >
            {type === 'confirm' ? 'ยืนยันรายการ' : 'ตกลง'}
          </button>
          ...
          {(type === 'confirm' || onCancel) && (
            <button
              ...
              disabled={loading}
              className="w-full py-3 text-[13px] font-bold text-text-muted hover:text-text-primary transition-colors"
            >
              ยกเลิก
            </button>
          )}
