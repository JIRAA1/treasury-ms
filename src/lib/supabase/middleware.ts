...
export async function updateSession(request: NextRequest) {
  ...
  // If user is logged in, prevent access to login or bind pages
  if (user && (url.pathname === '/login' || url.pathname === '/bind')) {
  ...
  if (!user && (url.pathname.startsWith('/student') || url.pathname.startsWith('/admin'))) {
    // Check if it's transparency which is public
    if (url.pathname !== '/student/transparency') {
      url.pathname = '/login'
      return NextResponse.redirect(url)
    }
  }
