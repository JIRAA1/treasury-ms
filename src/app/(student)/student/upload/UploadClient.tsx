...
export default function UploadClient({ profile, periods, payments, sysSettings, pendingCredits }: UploadClientProps) {
  ...
  const unpaidCycles = useMemo(() => {
    ...
    return unpaid
  }, [profile, periods, payments, sysSettings, pendingCredits])

  // Setup payable cycles
  const payableCycles = useMemo(() => {
    return unpaidCycles.filter(c => {
      const ws = getWindowStatus(c)
      return ws !== 'upcoming'
    })
  }, [unpaidCycles])

  // Initialize selected period ID and payAccumulated flag
  const [selectedPeriodId, setSelectedPeriodId] = useState<string | null>(() => {
    if (payableCycles.length > 0) {
      return payableCycles[0].id
    }
    return unpaidCycles.length > 0 ? unpaidCycles[0].id : null
  })

  const [payAccumulated, setPayAccumulated] = useState(() => {
  ...
  const accumulatedPeriodIds = useMemo(() => {
    return payableCycles.map(c => c.id)
  }, [payableCycles])
