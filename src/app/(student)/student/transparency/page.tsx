...
export default async function TransparencyPage() {
  ...
  try {
    ...
    const totalStudentIncome = payments?.reduce((s, p) => s + (p.amount || 0), 0) || 0
    const totalOtherIncome = incomes?.reduce((s, i) => s + (i.amount || 0), 0) || 0
    ...
    console.error('[TransparencyPage Error]', error)
    return (
      <div className="p-8 text-center min-h-screen flex flex-col items-center justify-center">
        <h2 className="text-xl font-bold text-red-600">เกิดข้อผิดพลาดในการโหลดข้อมูลความโปร่งใส</h2>
        <p className="text-text-muted mt-2">กรุณาลองใหม่อีกครั้ง หรือติดต่อเหรัญญิก</p>
        <a href="/student/transparency" className="mt-4 text-brand underline font-bold">ลองใหม่</a>
      </div>
    )
  }
}
